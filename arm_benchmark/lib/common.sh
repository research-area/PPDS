...
DATA_ROOT="${DATA_ROOT:-$ROOT_DIR/data/arm_benchmark}"
...
ensure_output_dir() {
  ...
  if ! mkdir -p "$out_dir"; then
    ...
    echo "[ERROR] Fix permissions or run with DATA_ROOT pointing to a writable path." >&2
    return 1
  fi
  printf "%s\n" "$out_dir"
}
...
ensure_server_ip() {
  ...
  if ! ip addr show dev "$iface" | grep -q "10.0.0.2/24"; then
    sudo ip addr add 10.0.0.2/24 dev "$iface" >/dev/null 2>&1 || true
  fi
}

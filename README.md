...
> **"PPDS: An Open-Source Dataset for Performance-Power trade-off analysis
Evaluation Across x86 and ARM Architectures"**

This project provides a client-server Java implementation of an **M/M/1-FCFS queuing system** used to measure and model the relationship between CORE utilization, frequency scaling (DVFS), and power consumption.
...
1. [Overview](#overview)
2. [Benchmark Guides and Dataset](#benchmark-guides-and-dataset)
   - [Dataset Columns](#dataset-columns)
3. [Architecture](#architecture)
4. [Hardware Preparation](#hardware-preparation)
...
6. [Monitoring](#monitoring)
   - [CORE Utilization](#cpu-utilization-mpstat)
...
| `duration` | Total duration for sending jobs to the server (milli-seconds)  |
| `repeat`   | Workload size per job; distributed exponentially between `repeat` and `1.33 × repeat` to simulate exponential service times |

### Server

The server (`Server.Server`) accepts incoming jobs over UDP, queues them, and processes them **one by one** on a single CPU core (M/M/1-FCFS discipline). After processing, it computes and records:
...
The `Job` class defines the structure of each work unit arriving from the client. Its `calc()` method performs a CPU-intensive calculation repeated proportionally to the job size (`repeat`). The workload is calibrated so that CORE utilization does not sustain 100% for more than ~1 second at the peak.

---

## Benchmark Guides and Dataset

For benchmark execution details, use the dedicated README files in each benchmark subfolder:

- `x86_benchmark/README.md`
- `arm_benchmark/README.md`

The dataset generated by PPDS is available in the `Dataset/` folder.

### Dataset Columns

The collected dataset comprises 19 metrics spanning governor configuration, theoretical queueing outputs, and empirical measurements.

- `CORE Utilization [%]`: Target CPU core utilization level used to configure each run.
- `Sampling Rate [μs]`: `ondemand` governor sampling interval used in that run.
- `Threshold [%]`: `ondemand` up-threshold value used in that run.
- `Modeled Arrival Rate [jobs/s]`: Theoretical arrival rate, $\lambda$, used to drive the experiment.
- `Measured Arrival Rate [jobs/s]`: Empirically observed arrival rate during execution.
- `Measured Mean Service Time [ms]`: Average time needed by the target core to process one job.
- `Measured Mean Service Rate [jobs/s]`: Empirical service rate, $\mu$, computed as the inverse of measured service time.
- `Modeled MRT (k/\lambda) [ms]`: Analytical mean response time estimated via $k/\lambda$.
- `Modeled MRT (W+S) [ms]`: Analytical mean response time estimated as waiting time plus service time, $W + S$.
- `Measured MRT [ms]`: Observed end-to-end response time (departure time minus arrival time).
- `Measured Mean Interarrival Time [ms]`: Average time between two consecutive arrivals (inverse of measured arrival rate).
- `Modeled Utilization [%]`: Queueing-model utilization, $\rho = (\lambda/\mu) \times 100$.
- `Modeled Busy Probability` $(1-\pi_0)$ `[%]`: Probability that the server is busy, computed from the empty-state probability:

$$
\pi_0 \approx \frac{f(s=0)}{N_{\text{states}}}, \qquad
(1-\pi_0)\times 100 = \left(1 - \frac{f(s=0)}{N_{\text{states}}}\right)\times 100
$$ 
where $f(s=0)$ is the number of observations in which the system is in state 0 (empty queue), and $N_{\text{states}}$ is the total number of observed states.

- `Measured Utilization [%]`: Observed utilization of the target core (obtained via `mpstat`).
...
| Listening ports | `9999` (UDP), `9950` (UDP) |
...
> **Working directory:** All commands below assume `./PPDS` as the working directory.
...
# java Server.Server <lambda> <duration>
# Example: lambda = 5 jobs/sec for 10min 
java Server.Server 5 600.0
```
...
| `repeat`   | `1000000`   | Base workload size per job; actual size is exponentially distributed between `repeat` and `1.33 × repeat` |
